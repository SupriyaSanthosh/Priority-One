page 60056 "Job Planning Lines1"
{
    AutoSplitKey = true;
        ...
        area(content)
        {
            repeater(Control1)
            {
                ...
                field(Description; Description)
                {
                    ...
                    trigger OnValidate()
                    begin
                        Validate("Work description", Description);
                    end;
            ...
            group("F&unctions")
            {
                ...
                action("Copy Planning Lines")
                {
                    ...
                    var
                        ...
                        Counter: Integer;
                        Answer: Boolean;
                        Question: Label 'Do you want to Copy Job Planning Lines?';
                    begin
                        If Not Dialog.Confirm(Question, true) then
                            exit;

                        Job.Get(Rec."Job No.");
                        Job.TestField("Next Action Date");
                        LineNo := GetLastLineNo(Job);

                        JobPlanningLine.SetCurrentKey("Job No.", "Line No.");
                        JobPlanningLine.SetRange("Job No.", Job."No.");
                        Counter := JobPlanningLine.Count;
                        if JobPlanningLine.FindSet() then
                            repeat
                                LineNo := LineNo + 10000;
                                NewJobPlanningLine.Init();
                                NewJobPlanningLine.TransferFields(JobPlanningLine, true);
                                NewJobPlanningLine."Line No." := LineNo;
                                NewJobPlanningLine.Validate("Planning Date", Job."Next Action Date");
                                NewJobPlanningLine.Insert(true);

                                if NewJobPlanningLine.Type <> NewJobPlanningLine.Type::Item then begin
                                    NewJobPlanningLine.Complete := true;
                                    NewJobPlanningLine.Modify();
                                end else begin
                                    if Item.Get(JobPlanningLine."Item No.") then begin
                                        if Item."Recurring Services" then begin
                                            NewJobPlanningLine.Complete := false;
                                            NewJobPlanningLine.Modify();
                                        end;
                                ...
                                Counter := Counter - 1;
                            until (JobPlanningLine.Next() = 0) or (Counter = 0);
                    end;
                }
    ...
    var
        Job: Record Job;
    begin
        "Line Type" := "Line Type"::"Both Budget and Billable";
        Type := Type::Item;
        Rec.SetRange(Complete, false);
        FilterGroup := 2;
    ...
    begin
        TestField("Line No.");
        Rec.SetCurrentKey("Job No.", "Line No.");
        JobPlanningLine.Copy(Rec);
        CurrPage.SetSelectionFilter(JobPlanningLine);
    ...
    local procedure GetLastLineNo(Job: Record Job): Integer
    var
        JobPlanningLines: Record "Job Planning Line";
    begin
        JobPlanningLines.SetCurrentKey("Job No.", "Line No.");
        JobPlanningLines.SetRange("Job No.", Job."No.");
        if JobPlanningLines.FindLast() then
            exit(JobPlanningLines."Line No.");
    end;

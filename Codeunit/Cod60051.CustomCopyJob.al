codeunit 60051 "Custom Copy Job"
{
    ...
    begin
        ...
        TargetJob."Legacy Code" := SourceJob."No.";
        if TargetJob.Frequency = TargetJob.Frequency::Annually then begin
            TargetJob."Starting Date" := CalcDate('<+1Y>', TargetJob."Starting Date");
            TargetJob."Ending Date" := CalcDate('<+1Y>', TargetJob."Ending Date");
        end;
        if TargetJob.Frequency = TargetJob.Frequency::Monthly then begin
            TargetJob."Starting Date" := CalcDate('<+1M>', TargetJob."Starting Date");
            TargetJob."Ending Date" := CalcDate('<+1M>', TargetJob."Ending Date");
        end;
        if TargetJob.Frequency = TargetJob.Frequency::Quaterly then begin
            TargetJob."Starting Date" := CalcDate('<+3M>', TargetJob."Starting Date");
            TargetJob."Ending Date" := CalcDate('<+3M>', TargetJob."Ending Date");
        end;
    ...
    begin
        ...
        SourceJobPlanningLine.SetRange(Type, SourceJobPlanningLine.Type::Item);
        if ItemCatCode <> '' then
            SourceJobPlanningLine.SetRange("Item Category Code", ItemCatCode);
        ...
        if SourceJobPlanningLine.FindSet then
            repeat
                with TargetJobPlanningLine do begin
                    ...
                    if SourceJob.Frequency = SourceJob.Frequency::Annually then
                        "Planning Date" := CalcDate('<+1Y>', "Planning Date");

                    if SourceJob.Frequency = SourceJob.Frequency::Monthly then
                        "Planning Date" := CalcDate('<+1M>', "Planning Date");

                    if SourceJob.Frequency = SourceJob.Frequency::Quaterly then
                        "Planning Date" := CalcDate('<+3M>', "Planning Date");
                    //MQ-SUPRIYA-14-09-2021 <<
    ...
    begin
        ...
        if ItemCatCode <> '' then
            JobLedgEntry.SetRange("Item Category Code", ItemCatCode);
        case true of
    ...
    procedure SetCopyOptions(CopyPrices2: Boolean; CopyQuantity2: Boolean; CopyDimensions2: Boolean; JobPlanningLineSource2: Option "Job Planning Lines","Job Ledger Entries"; JobPlanningLineType2: Option " ",Budget,Billable; JobLedgerEntryType2: Option " ",Usage,Sale; ItemCatCode2: Code[50])
    begin
